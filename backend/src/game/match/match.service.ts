...
export class MatchService {
  ...
  ballMovement() {
    const ballPosY: number =
      this.gameData.ballY + 100 / 2; /* 100 == ball.height */
    const racketLeftPosY: number =
      this.gameData.racketLeftY + 160 / 2; /* 160 == racketLeft.height*/
    const racketRightPosY =
      this.gameData.racketRightY + 160 / 2; /* 160 == racketRight.height*/
    ...
    if (this.gameData.ballX <= 70) {
      ...
      if (this.ballMoveSpeed < 70) this.ballMoveSpeed += 1;
    }
    ...
    if (this.gameData.ballX >= 0 && this.gameData.ballX <= 924) {
      this.gameData.ballX +=
        Math.sin(this.radInDegrees(this.ballMoveDegree)) * this.ballMoveSpeed;
      this.gameData.ballY +=
        Math.cos(this.radInDegrees(this.ballMoveDegree)) * this.ballMoveSpeed;
    }
  }
  ...
  setUsers(leftUserID: number, rightUserID: number) {
    ...
    console.log(
      'in setUsers the leftUserID in GameData is:   ',
      this.gameData.leftUserID,
    );
  }
}
