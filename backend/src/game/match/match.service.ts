...
export class MatchService {
	...
	ballMovement(gameData: gameData) : gameData {
		...
		if (gameData.ballX <= 70) {
			...
			if (gameData.ballMoveSpeed < 70)
				gameData.ballMoveSpeed += 1;
		}
		...
		if (gameData.ballX >= 0 && gameData.ballX <= 924) {
			gameData.ballX += Math.sin(this.radInDegrees(gameData.ballMoveDegree)) * gameData.ballMoveSpeed;
			gameData.ballY += Math.cos(this.radInDegrees(gameData.ballMoveDegree)) * gameData.ballMoveSpeed;
		}
		return (gameData);
	}
