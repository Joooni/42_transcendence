...
export class ChannelsService {
	...
	async createChannel(client: Socket, channelname: string, owner: number) {
		...
		user.ownedChannels = user.ownedChannels ? [...user.ownedChannels, chanEntity] : [chanEntity];
		user.channelList = user.channelList ? [...user.channelList, chanEntity] : [chanEntity];
		user.adminInChannel = user.adminInChannel ? [...user.adminInChannel, chanEntity] : [chanEntity];
	...
	async removeUserFromChannel(client: Socket, channelId: string, userid: number) {
		try {
			const channel = await this.channelRepository.findOneByOrFail({ id: channelId });
			const user = await this.userService.findOne(userid);
			if (!channel || !user) {
			...
			user.channelList = user.channelList.filter((channel) => channel.id !== channelId);
